...
### Spurious characters
I believe that the invertor (and possibly the Wifi logger as well) is generating spurious characters on the serial line in or around the time it responds to a request. This can be seen in the [sample log file](data/2024-11-08_12-07-44.log) where it reports things like _Skipped 2 bytes in stream looking for next header_. In the case of the RS485/USB adaptor used during the first part of the project, this seems to materialise as up to 3 NULL bytes which I believe are actually _serial break_ characters (or framing errors). Where the ESP-32 module is concerned, I see just random characters. I've no obvious explanation for this, neither adding in bias or termination resistors has made any difference (and based on the behaviour, suspected they woudn't). 

As a result, I've had to implement software workarounds, in effect to discard any incoming bytes up until the expected slave-id. In the case of the Linux applications, this is in the form of a patch against the current. 3.1.11 release of the libmodbus library, this can be found in the [libmodbus folder](libmodbus/). For the ESP-32 module, I've created a fork of the [/fridgemagnet3/ModbusMaster]([https://github.com/fridgemagnet3/solar-display-micropython](https://github.com/fridgemagnet3/ModbusMaster)) library.
